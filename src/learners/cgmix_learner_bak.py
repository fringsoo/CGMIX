...
class CgmixLearner():
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        for t in range(batch.max_seq_length - 1):
            ...
            #f_i = self.mac.forward(batch, t=t)
        ...
        for t in range(batch.max_seq_length):
            ...
            #f_i= self.target_mac.forward(batch, t=t)
        ...
        '''
        #mac_out = mac_f_i
        #target_out = target_f_i

        #import pdb; pdb.set_trace()

        chosen_action_qvals = th.gather(mac_out, dim=3, index=actions).squeeze(3)  # Remove the last dim
        # Mask out unavailable actions
        target_out[avail_actions[:, 1:] == 0] = -9999999
        target_max_qvals = target_out.max(dim=3)[0]

        # Mix
        if self.mac.mixer is not None:
            chosen_action_qval = self.mac.mixer(chosen_action_qvals, batch["state"][:, :-1])
            target_max_qvals = self.target_mac.mixer(target_max_qvals, batch["state"][:, 1:])
        '''
        ...
        if t_env - self.log_stats_t >= self.args.learner_log_interval:
            ...
            self.logger.log_stat("q_taken_mean", (mac_out * mask).sum().item()/(mask_elems * self.args.n_agents), t_env) #chosen_action_qvals
